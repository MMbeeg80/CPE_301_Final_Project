...
#define STEPPER_PIN1 31
#define STEPPER_PIN2 33
#define STEPPER_PIN3 35
...
//Define Serial Connection registers
...
#define WATER_THRESHOLD 250 // the threshold for that sensor
#define TEMP_THRESHOLD 26 // the temp threshold for that sensor
...
void setup()
{
  ...
  //inputs
  DDRE |=(0x00 << BUTTON_RESET | 0x00 << BUTTON_ON_OFF | 0x00 << BUTTON_STEPPER_DOWN);
  DDRG |=(0x00 << BUTTON_STEPPER_UP);

  //outputs
  DDRH |= (0x01 << MOTOR_PIN);
  DDRL |= (0x01 << LED_PINB | 0x01 << LED_PINY | 0x01 << LED_PINY | 0x01 << LED_PINY);
...
void loop()
{
  ...
  if(stepperAllowed){
    ...
    int stepperDirection = stepperRate * (stepUpRead(BUTTON_STEPPER_UP) ? 1 : stepDownRead(BUTTON_STEPPER_DOWN) ? -1 : 0);
...
int pinRead(int pin) {
  ...
  return PINE & (0x01 << pin);
}

int stepUpRead(int pin){
  return PING & (0x01 <<pin);
}

int stepDownRead(int pin){
  return PINE & (0x01 <<pin);
}

volatile bool buttonOnState = true;
